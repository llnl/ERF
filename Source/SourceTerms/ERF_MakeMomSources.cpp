                       ...
                       int n_qstate)
{
    ...
    for ( MFIter mfi(S_data[IntVars::cons]); mfi.isValid(); ++mfi)
    {
        ...
        // *****************************************************************************
        /*
        if (geo_wind_profile && solverChoice.nudging_from_input_sounding) {
            ...
            ParallelFor(tbx, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                ...
                //Real nudge_u = tau_inv * (dptr_u_plane(k) / dptr_r_plane(k) - dptr_u_geos[k]);
                
                Real nudge_u = -((dptr_u_plane(k) / dptr_r_plane(k)) - dptr_u_geos[k]);
                nudge_u *= tau_inv;
